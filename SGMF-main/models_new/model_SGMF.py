...
class SGMF(torch.nn.Module):
    ...
    def forward(self, data_s, data_l, tissue_data, attn_mask_s, attn_mask_l, attention_only=False):
        ...
        tmp_mask_s = torch.eye(tissue_feature.shape[0], device=device).unsqueeze(0) 
        ...
        tmp_mask_l = torch.eye(tissue_feature.shape[0], device=device).unsqueeze(0) 
        ...
        logits = self.classifier(h_fusion).squeeze(0)
        save_logits = logits

        Y_prob = F.softmax(logits, dim = 1)
        Y_hat = torch.topk(logits, 1, dim = 1)[1]

        return logits, Y_prob, Y_hat, h_fusion

...
class Attn_Net_Gated(nn.Module):
    ...
    def forward(self, x):
        a = self.attention_a(x) # N x 512
        b = self.attention_b(x) # N x 512
        A = a.mul(b) # N x 512
        A = self.attention_c(A)
        # A = self.softmax(A)  # N x n_classes
        return A, x
        # attention_weights = F.softmax(A, dim=1)   # N x n_classes
...
def multi_head_attention_forward(
    ...
    if attn_mask is not None:
        ...
        assert (
                ...
                or attn_mask.dtype == torch.int64
                or attn_mask.dtype == torch.int32
                or attn_mask.dtype == torch.int16
                or attn_mask.dtype == torch.uint8
                or attn_mask.dtype == torch.bool
        ), "Only int, float, byte, and bool types are supported for attn_mask, not {}".format(attn_mask.dtype)

...
class SGMF(torch.nn.Module):
    def __init__(self, input_dim=2227, num_layers=4, edge_agg='spatial', multires=False, resample=0,
        ...
        self.tissue_graph = Tissue_GCN_cls().to(torch.device("cpu"))
    ...
    def forward(self, data_s, data_l, tissue_data, attn_mask_s, attn_mask_l, attention_only=False):
        ...
        # gcn for small scale
        x_s = self.fc(data_s.x) #对data_s的特征进行线性变换，通过全连接层（self.fc）将特征维度调整为128维
        x1_s = x_s
        x_s = self.layers_s[0].conv(x1_s, edge_index_s, edge_attr_s)
        x1_s = torch.cat([x1_s, x_s], axis=1)
        for layer_s in self.layers_s[1:]: #通过一系列的GCN层，之后进行拼接
            x_s = layer_s(x_s, edge_index_s, edge_attr_s)
            x1_s = torch.cat([x1_s, x_s], axis=1)  # concate every layer's output together
        h_path_s = x1_s
        h_path_s = self.path_phi_s(h_path_s).unsqueeze(0)  # each node's feature  # 1xNx(4xhidden)
        ...
        tissue_feature = tissue_feature.unsqueeze(1) # Nx1x512

        # self_attention for small and large patches
        h_path_s = torch.cat((tissue_feature.permute(1,0,2), h_path_s), dim=1)
        tmp_mask_s = torch.eye(tissue_feature.shape[0], device=torch.device("cpu")).unsqueeze(0)
        tmp_mask_s = ~ tmp_mask_s.type(torch.bool) #(1 x Nt x Nt)
        attn_mask_s = attn_mask_s.reshape(1, -1, attn_mask_s.shape[-1])
        attn_mask_s = attn_mask_s.type(torch.bool)
        attn_mask_s_new = torch.cat((tmp_mask_s, attn_mask_s), dim=-1)

        h_path_l = torch.cat((tissue_feature.permute(1,0,2), h_path_l), dim=1)
        tmp_mask_l = torch.eye(tissue_feature.shape[0], device=torch.device("cpu")).unsqueeze(0)
        tmp_mask_l = ~ tmp_mask_l.type(torch.bool)
        attn_mask_l = attn_mask_l.reshape(1, -1, attn_mask_l.shape[-1])
        attn_mask_l = attn_mask_l.type(torch.bool)
        ...
        # Nx1x512
        h_fusion = torch.cat((h_path_cooatn_s, h_path_cooatn_l), dim=-1).squeeze(0) # Nx1x1024
        tissue_data.x = h_fusion.squeeze() 
        h_fusion, tissue_edge_index = self.tissue_graph(tissue_data)# Nx512


        A_fusion, h_fusion = self.path_attention_head(h_fusion) #Nx1 Nx512
        ...
        return logits, Y_prob, Y_hat, A_fusion
